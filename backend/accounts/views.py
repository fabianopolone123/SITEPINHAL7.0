...
def _classe_aventureiro_por_idade(nascimento, hoje=None):
    ...
    # Regra solicitada: abaixo de 6 anos, sempre Abelhinhas.
    if idade_atual < 6:
...
class EventoPublicoView(View):
    ...
    def _evento_bruto_antes_estorno_atual(self, evento):
        inscricoes_excluir_teste_ids = _evento_inscricoes_excluir_teste_ids(evento=evento)
        inscricoes_valor_pago = (
            EventoInscricao.objects
            .filter(evento=evento, confirmada=True, cancelada=False)
            .exclude(id__in=list(inscricoes_excluir_teste_ids))
            .aggregate(total=Sum('valor_inscricao'))
        ...
        return (
            Decimal(inscricoes_valor_pago)
            + Decimal(inscricoes_valor_estornado)
            + Decimal(pedidos_total_pago)
        ).quantize(Decimal('0.01'))
        ...
        if can_manage_evento:
            try:
                for produto_row in produtos:
                    ...
                    if not produto_obj or not getattr(produto_obj, 'ativo', True):
                        continue
                    for variacao in (produto_row.get('variacoes') or []):
                        if not getattr(variacao, 'ativo', True):
                            continue
                        estoque_raw = ''
    ...
    def _post_impl(self, request, event_id):
        ...
        if action == 'registrar_venda_evento':
        ...
        if action == 'calcular_taxa_cartao_evento':
            ...
            messages.success(
                ...
                (
                    ...
                    + (
                        f' | Total liquido geral desejado: {self._format_currency(total_liquido_geral_desejado)}.'
            ...
            if field['type'] == 'repetidor':
                ...
                for row in repeat_schema_rows:
                    ...
                    if normalized_key in {
                        self._event_repeat_discount_field_key(evento),
                        self._event_repeat_diretoria_field_key(evento),
                    }:
    ...
    def _cobranca_mensalidades_groups(self, aventureiro_id=None):
        ...
        for group in grouped.values():
            ...
            groups.append({
                ...
                'total_em_aberto_fmt': self._format_currency(total_em_aberto),
                'itens_count': len(items),
                'itens_preview': [
                    {
                        'mensalidade_id': item.pk,
            ...
            'groups': [
                {
                    ...
                    'responsavel_nome': group['responsavel_nome'],
                    'phone_number': group['phone_number'],
                    'has_user': group['has_user'],
                    'has_phone': group['has_phone'],
                    'itens_count': group['itens_count'],
                    'total_em_aberto': group['total_em_aberto_fmt'],
                    'itens_preview': group['itens_preview'],
                }
                for group in groups
            ],
        }
    ...
    def _send_whatsapp_cobranca_group(self, responsavel_id, aventureiro_id=None):
        ...
        if not group:
            return {
                'ok': False,
                'status': 'skipped',
                'message': 'Nao ha mensalidades pendentes elegiveis para este responsavel.',
                'sent_mensalidades': 0,
                'failed_mensalidades': 0,
            }

        responsavel = group['responsavel']
        responsavel_user = group['responsavel_user']
        responsavel_nome = group['responsavel_nome']
        if not responsavel_user:
            return {
                'ok': False,
                'status': 'sem_usuario',
                'message': 'Responsavel sem usuario vinculado.',
                'responsavel_id': group['responsavel_id'],
                'responsavel_nome': responsavel_nome,
                'sent_mensalidades': 0,
                'failed_mensalidades': 0,
        ...
        if not phone_number:
            return {
                'ok': False,
                'status': 'sem_telefone',
                'message': 'Responsavel sem telefone WhatsApp valido.',
                'responsavel_id': group['responsavel_id'],
                'responsavel_nome': responsavel_nome,
                'sent_mensalidades': 0,
                'failed_mensalidades': 0,
        ...
        with transaction.atomic():
            ...
            if not locked_items:
                return {
                    'ok': False,
                    'status': 'skipped',
                    'message': 'As mensalidades deste responsavel nao estao mais pendentes.',
                    'responsavel_id': group['responsavel_id'],
                    'responsavel_nome': responsavel_nome,
                    'sent_mensalidades': 0,
                    'failed_mensalidades': 0,
            ...
            MensalidadeAventureiro.objects.filter(pk__in=[item.pk for item in locked_items]).update(
        ...
        message_text = render_message(template, {
            'responsavel_nome': responsavel_nome or responsavel_user.username,
            ...
            'mes_referencia': f'{self._month_label(hoje.month)}/{hoje.year}',
        ...
        return {
            'ok': False,
            'status': 'failed',
            'message': error_message or 'Falha no envio.',
            'responsavel_id': group['responsavel_id'],
            'responsavel_nome': responsavel_nome,
            'phone_number': phone_number,
            'sent_mensalidades': 0,
    ...
    def _send_whatsapp_cobranca_mensalidades(self, pause_seconds=4, aventureiro_id=None):
        ...
        for group in groups:
            ...
            if result.get('status') == 'sem_usuario':
                ...
                skipped_sem_telefone_labels.append(result.get('responsavel_nome') or '-')
            elif result.get('ok'):
                sent_responsaveis += 1
                sent_mensalidades += int(result.get('sent_mensalidades') or 0)
            elif result.get('status') == 'failed':
                failed_responsaveis += 1
                failed_mensalidades += int(result.get('failed_mensalidades') or 0)
...
class LojaView(LoginRequiredMixin, View):
    ...
    def _produto_rows(self, *, only_active=False, only_active_variacoes=False):
        ...
        for produto in produtos:
            ...
            fotos_por_variacao = {}
            variacao_ids_produto = {v.id for v in variacoes}
            for foto in fotos:
                if getattr(foto, 'todas_variacoes', False):
                    destino_ids = set(variacao_ids_produto)
                else:
                    destino_ids = {foto.variacao_id} if foto.variacao_id else set()
                    destino_ids.update(v.id for v in foto.variacoes_vinculadas.all())
                    destino_ids &= variacao_ids_produto
                for variacao_id in destino_ids:
    ...
    def post(self, request):
        ...
        if action == 'editar_config_variacoes_produto':
            ...
            orientacoes_produto = str(request.POST.get('config_orientacoes_produto') or '').strip()
            ...
            rows_total = max(len(variacao_ids_raw), len(obrigatoria_compra_raw))
            if rows_total <= 0:
        ...
        descricao = str(request.POST.get('descricao') or '').strip()
        orientacoes_produto = str(request.POST.get('orientacoes_produto') or '').strip()
        ...
        fotos_parsed = []
        max_fotos_len = max(len(foto_row_ids), 1)
        for idx in range(max_fotos_len):
            ...
            variacao_refs = [str(v).strip() for v in request.POST.getlist(f'foto_variacoes__{row_id}[]') if str(v).strip()]
            form_data['fotos'].append({
                'row_id': row_id,
                'variacao_refs': variacao_refs,
                'variacao_refs_csv': ','.join(variacao_refs),
            })

            if not arquivo and not variacao_refs:
                continue
            if not arquivo:
                ...
                context.update(_sidebar_context(request))
                return render(request, self.template_name, context)
            if not variacao_refs:
                messages.error(request, f'Selecione pelo menos uma variação para a foto na linha {idx + 1}.')
                context = self._context(form_data=form_data)
                context.update(_sidebar_context(request))
                return render(request, self.template_name, context)
            refs_unicos = []
            seen_refs = set()
            for ref in variacao_refs:
                if ref in seen_refs:
                    continue
                seen_refs.add(ref)
                refs_unicos.append(ref)
            if 'all' in refs_unicos:
                variacao_indices = list(range(len(variacoes_parsed)))
                todas_variacoes = True
            else:
                variacao_indices = []
                for ref in refs_unicos:
                    if not re.fullmatch(r'v\d+', ref):
                        ...
                        context.update(_sidebar_context(request))
                        return render(request, self.template_name, context)
                    variacao_index = variacao_ref_to_parsed_index.get(ref)
                    if variacao_index is None:
                        messages.error(request, f'A foto da linha {idx + 1} está vinculada a uma variação inexistente.')
                        context = self._context(form_data=form_data)
                        context.update(_sidebar_context(request))
                        return render(request, self.template_name, context)
                    if variacao_index not in variacao_indices:
                        variacao_indices.append(variacao_index)
                todas_variacoes = False
            if not variacao_indices:
                messages.error(request, f'Selecione pelo menos uma variação válida para a foto na linha {idx + 1}.')
                context = self._context(form_data=form_data)
                context.update(_sidebar_context(request))
                return render(request, self.template_name, context)
            fotos_parsed.append({
                'arquivo': arquivo,
                'variacao_indices': variacao_indices,
                'todas_variacoes': todas_variacoes,
        ...
        with transaction.atomic():
            ...
            if fotos_parsed:
                for item in fotos_parsed:
                    variacoes_destino = [variacoes_criadas[i] for i in item['variacao_indices']]
                    foto_obj = LojaProdutoFoto.objects.create(
                        produto=produto,
                        variacao=variacoes_destino[0],
                        todas_variacoes=item['todas_variacoes'],
                        foto=item['arquivo'],
                        ordem=item['ordem'],
                    )
                    foto_obj.variacoes_vinculadas.set(variacoes_destino)
